...
/**
 * [클래스 역할] - 플레이어 충돌 감지 서비스(백그라운드 계속 돌아감)
 * 메인 스레드는 너무 바쁨
 */
public class BackgroundPlayerService implements Runnable {
    ...
    // BufferedImage
    // >> 메모리에 픽셀 배열로 저장된 이미지
    // >> getRGB(x,y)로 특정 좌표의 색상 값을 직접 읽을 수 있다.
    ...
    // 즉, 이 서비스가 직접 플레이어를 생성하지 않고 외부에서 주입받아 사용됨.
    ...
    public void run() {
        ...
        while (true) {
            ...
            if(isRed(rightColor)) {
                ...
                player.setLeftWallCrash(false); // 벽에서 벗어나면 즉시 해제
            }
