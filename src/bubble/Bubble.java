...
public class Bubble extends JLabel implements Moveable {
    ...
    // 버블 이동 상태 플래그
    private static final int HORIZONTAL_DISTANCE = 400; // 수평 이동거리
    ...
    public Bubble(Player player) {
        ...
        initData();
        setInitLayout();
        bubbleStartThread(); // 생성과 동시에 플레이어 방향 판단해서 바로 이동시작
    }
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                if (player.getPlayerWay() == PlayerWay.LEFT) {
                    left(); // 왼쪽으로 400px 이동 >> 완료후 up() 호출
                } else {
                    right(); // 오른쪽으로 400px 이동 >> 완료후 up() 호출
                }
            }
        }).start();
    }
